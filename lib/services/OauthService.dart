...
class OauthService {
  ...
  Future<oauth2.Client> getClient(oauth2.Credentials credentials) async {
    this.oauthClient =
        oauth2.Client(credentials, identifier: this.instanceSettings.clientID);
  ...
  refreshToken() async {
    ...
    await getIt<PreferencesService>()
        .oAuthCredentialsAsync(this.oauthClient.credentials);
  ...
  Future<oauth2.Client> authorize() async {
    ...
    var grant = oauth2.AuthorizationCodeGrant(
        this.instanceSettings.clientID,
        Uri.parse(this.instanceSettings.authURL),
        Uri.parse(this.instanceSettings.tokenURL));

    // A URL on the authorization server (authorizationEndpoint with some additional
    // query parameters). Scopes and state can optionally be passed into this method.
    var authorizationUrl = grant.getAuthorizationUrl(
        Uri.parse(this.instanceSettings.redirectURL),
  ...
  Future<Stream<Map<String, String>>> _server() async {
    ...
    HttpServer server = await HttpServer.bind(
        InternetAddress.loopbackIPv4, this.instanceSettings.loopBackPort);
